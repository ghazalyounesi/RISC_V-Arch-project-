...
Encoder::Encoder() {
    ...
    // *** خطوط جدید برای رفع مشکل اینجا اضافه شده‌اند ***
...
uint32_t Encoder::encode_i_type(const InstructionInfo& info, const std::vector<std::string>& operands) {
    uint32_t rd = Utils::register_to_int(operands[0]);
    uint32_t rs1;
    int32_t imm; // از نوع علامت‌دار برای مقادیر منفی

    // تشخیص فرمت عملوندها: addi rd, rs1, imm  یا  lw rd, offset(rs1)
    ...
    } else { // فرمت استاندارد با مقدار ثابت
        rs1 = Utils::register_to_int(operands[1]);
        imm = Utils::string_to_int(operands[2]);
    }

    std::string imm_field;

    // *** بخش اصلاح شده برای مدیریت دستورات شیفت ***
    // این دستورات را از روی opcode و funct3 شناسایی می‌کنیم
    if (info.opcode == "0010011" && (info.funct3 == "001" || info.funct3 == "101")) {
        // برای دستورات شیفت، فیلد immediate از funct7 و shamt تشکیل شده
        uint32_t shamt = imm & 0x1F; // مقدار شیفت (shamt) فقط ۵ بیت است
        imm_field = info.funct7 + std::bitset<5>(shamt).to_string();
    } else {
        // برای سایر دستورات نوع I، فیلد immediate یک عدد ۱۲ بیتی است
        imm_field = std::bitset<12>(imm).to_string();
    }
    // *** پایان بخش اصلاح شده ***

    std::string binary_str = imm_field +
                             std::bitset<5>(rs1).to_string() +
                             info.funct3 +
                             std::bitset<5>(rd).to_string() +
                             info.opcode;
...
// پیاده‌سازی توابع کمکی که در common.h تعریف شده‌اند
...
// سازنده Encoder با تمام دستورالعمل‌های جدید از فایل RTL
Encoder::Encoder() {
    ...
    // I-Type
    instruction_map["addi"]  = {"0010011", "000", "", 'I'};
    instruction_map["slti"]  = {"0010011", "010", "", 'I'}; // ADDED
    instruction_map["sltiu"] = {"0010011", "011", "", 'I'}; // ADDED
    instruction_map["xori"]  = {"0010011", "100", "", 'I'};
    instruction_map["ori"]   = {"0010011", "110", "", 'I'};
    instruction_map["andi"]  = {"0010011", "111", "", 'I'};
    // I-Type (Shifts) - funct7 is required
    instruction_map["slli"]  = {"0010011", "001", "0000000", 'I'}; // ADDED
    instruction_map["srli"]  = {"0010011", "101", "0000000", 'I'}; // ADDED
    instruction_map["srai"]  = {"0010011", "101", "0100000", 'I'}; // ADDED
    ...
    // R-Type
    instruction_map["mul"]    = {"0110011", "000", "0000001", 'R'};
    instruction_map["mulh"]   = {"0110011", "001", "0000001", 'R'}; // ADDED
    instruction_map["mulhsu"] = {"0110011", "010", "0000001", 'R'}; // ADDED
    instruction_map["mulhu"]  = {"0110011", "011", "0000001", 'R'}; // ADDED
    instruction_map["div"]    = {"0110011", "100", "0000001", 'R'};
    instruction_map["divu"]   = {"0110011", "101", "0000001", 'R'}; // ADDED
    instruction_map["rem"]    = {"0110011", "110", "0000001", 'R'};
    instruction_map["remu"]   = {"0110011", "111", "0000001", 'R'}; // ADDED
}
...
// تابع اصلاح شده برای پشتیبانی از فرمت‌های مختلف I-Type
uint32_t Encoder::encode_i_type(const InstructionInfo& info, const std::vector<std::string>& operands) {
    ...
    if (info.opcode == "0000011" || info.opcode == "1100111") { // Load or JALR
        ...
        // فرمت Shift: slli rd, rs1, shamt
    else if (info.opcode == "0010011" && (info.funct3 == "001" || info.funct3 == "101")) { // slli, srli, srai
        ...
        // funct7 برای srai از مپ خوانده می‌شود و برای slli/srli صفر است
        imm_binary_str = info.funct7 + std::bitset<5>(shamt).to_string();
    }
        // فرمت استاندارد immediate: addi rd, rs1, imm
    else {
